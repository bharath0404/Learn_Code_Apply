...
def click_event(event, x, y, flags, param):
    if event == cv2.EVENT_LBUTTONDOWN: #For left-click event, display the RGB color quantifier on clicked point  
        ...
        cv2.putText(img, strT, (x,y),cv2.FONT_HERSHEY_SIMPLEX,0.5,(255,255,0),2)
        cv2.imshow('Mouse Events',img)
    
    if event == cv2.EVENT_RBUTTONDOWN: # For right click event, draw a line using points clicked using right mouse button.
...
img = cv2.imread('/home/bharath/Learn_Code_Apply/OpenCV/lena.jpg',1)
points = [] # Use an array to store the clicked points
cv2.imshow('Mouse Events',img)
cv2.setMouseCallback('Mouse Events',click_event) # Command to use the mouse callback function 
